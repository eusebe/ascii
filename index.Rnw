...
* a simple wrapper for +Sweave()+ names +SweaveAscii()+ that uses +RweaveAsciidoc()+ as default.
...
+list.type+:: Character vector of length one indicating the list type
  (+"bullet"+, +"number"+, "label" or +"none"+). If +"label"+, 
  +names(list)+ is used for labels. Default is +"bullet"+.
