...
ascii(toto[, -1], lgroup = c("Treatment", paste("Treatment:", as.character(unique(toto[, 1])))), n.lgroup = c(1, table(toto[, 1])), rstyle = "s", lvalign = "middle")
@ 
...
ff_d <- melt(french_fries, id=1:4, na.rm=TRUE)
toto <- cast(ff_d, treatment + subject ~ variable, mean, margins="treatment")
ascii(toto[, -1], lgroup = c("Treatment", paste("Treatment:", as.character(unique(toto[, 1])))), n.lgroup = c(1, table(toto[, 1])), lstyle = "s", lvalign = "middle")
@ 
