...
RweavePandoc <- function()
{
    list(setup = RweavePandocSetup,
         runcode = RweavePandocRuncode,
         writedoc = RweavePandocWritedoc,
         finish = RweavePandocFinish,
...
##' @param ... ...
##' @keywords internal
RweavePandocSetup <- function(file, syntax, output=NULL, quiet=FALSE, debug=FALSE,
             stylepath, ...)
{
    dots <- list(...)
    if(is.null(output)) {
        prefix.string <- basename(sub(syntax$extension, "", file))
        output <- paste(prefix.string, "md", sep=".")
    } else prefix.string <- basename(sub("\\.md$", "", output))
    ...
    options <- RweavePandocOptions(options)
...
##' RweavePandocCodeRunner
##'
##' @param evalFunc evalFunc
##' @keywords internal
makeRweavePandocCodeRunner <- function(evalFunc=RweaveEvalWithOpt)
{
    ...
    RweavePandocRuncode <- function(object, chunk, options)
      {
          ...
          if(options$split){
              ...
              if(is.null(chunkout)){
                  chunkout <- file(paste(chunkprefix, "md", sep="."), "w")
                  if(!is.null(options$label))
...
RweavePandocRuncode <- makeRweavePandocCodeRunner()

##' RweavePandocWritedoc
##'
##' @param object object
##' @param chunk chunk
##' @keywords internal
RweavePandocWritedoc <- function(object, chunk)
{
...
##' @param error error
##' @keywords internal
RweavePandocFinish <- function(object, error=FALSE)
{
    outputname <- summary(object$output)$description
    inputname <- object$srcfile$filename
    if(!object$quiet && !error)
        cat("\n",
            gettextf("You can now run Pandoc on '%s'", outputname),
            "\n", sep = "")
    close(object$output)
...
##' @keywords internal
RweavePandocOptions <- function(options)
{
...
##'
##' @keywords internal
RtanglePandoc <-  function()
{
    list(setup = RtanglePandocSetup,
         runcode = RtanglePandocRuncode,
         writedoc = RtanglePandocWritedoc,
         finish = RtanglePandocFinish,
         checkopts = RweavePandocOptions)
}
...
##' @param quiet quiet
##' @keywords internal
RtanglePandocSetup <- function(file, syntax,
...
##' RtanglePandocRuncode
##'
##' @param object object
##' @param chunk chunk
##' @param options options
##' @keywords internal
RtanglePandocRuncode <-  function(object, chunk, options)
{
    if(!(options$engine %in% c("R", "S"))){
        return(object)
    }
...
##' RtanglePandocWritedoc
##'
##' @param object object
##' @param chunk chunk
##' @keywords internal
RtanglePandocWritedoc <- function(object, chunk)
{
    while(length(pos <- grep(object$syntax$docopt, chunk)))
    {
        opts <- sub(paste(".*", object$syntax$docopt, ".*", sep=""),
...
##' @param error error
##' @keywords internal
RtanglePandocFinish <- function(object, error=FALSE)
{
